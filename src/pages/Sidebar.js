  ...
  return (
    <div className={`${isOpen ? 'right-0' : '-right-full'} w-full bg-white fixed top-20 h-full shadow-2xl md:w-[35vw] xl:max-w-[30vw] transition-all duration-300 z-20 px-4 lg:px-[35px]`}>
      <div className='flex items-center justify-between py-6 border-b'>
        <div className='uppercase text-gray-700 text-sm font-semibold'>Shopping Bag ({itemAmount})</div>
        <div onClick={handleClose} className='cursor-pointer w-8 h-8 flex justify-center text-gray-700 items-center'>
      ...
      <div className='flex flex-col gap-y-3 py-4 mt-4'>
        <div className='flex w-full justify-between items-center'>
          <div className='uppercase font-semibold text-gray-700'>
            <span className='mr-2 text-gray-700'>Total:</span> $ {parseFloat(total).toFixed(2)}
          </div>
